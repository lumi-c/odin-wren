...
ReallocateFn :: #type proc(memory: rawptr, newSize: u64, userData: rawptr) -> rawptr;
ForeignMethodFn :: #type proc(vm: ^VM);
FinalizerFn :: #type proc(data: rawptr);
ResolveModuleFn :: #type proc(vm: VM, importer: cstring, name: cstring) -> cstring;
LoadModuleCompleteFn :: #type proc(vm: ^VM, name: cstring, result: LoadModuleResult);
LoadModuleFn :: #type proc(vm: ^VM, name: cstring) -> LoadModuleResult;
BindForeignMethodFn :: #type proc(vm: ^VM, module: cstring, className: cstring, isStatic: bool, signature: cstring) -> ForeignMethodFn;
